    ...
    describe('mixed test vector', function() {
        ...
        function test(opt) {
            it('should not fail at ' + opt.name, function() {
                let h = new Hmac(Sha256, opt.key, 'hex');
                Assert.assertEqual(h.update(opt.msg, opt.msgEnc).digest('hex'), opt.res);

                h = new Hmac(Sha256, opt.key, 'hex')
                Assert.assertEqual(h
