...
import { Hash, ripemd160, sha1, sha224, sha256, sha384, sha512 } from '../hash';

describe('Hash', function () {

    function test(hashAlgorithm: () => BlockHash, cases) {
        for (var i = 0; i < cases.length; i++) {
            var msg = cases[i][0];
            var res = cases[i][1];
            var enc = cases[i][2];

            var dgst = hashAlgorithm().update(msg, enc).digest('hex');
            Assert.assertEqual(dgst, res);

            // Split message
            dgst = hashAlgorithm().update(msg.slice(0, 2), enc)
    ...
    it('should support sha256', function () {
        ...
        Assert.assertEqual(algorithm.blockSize, 512);
        Assert.assertEqual(algorithm.outSize, 256);
    ...
    it('should support sha224', function () {
        let algorithm = sha224();
        Assert.assertEqual(algorithm.blockSize, 512);
        Assert.assertEqual(algorithm.outSize, 224);
    ...
    it('should support ripemd160', function () {
        let algorithm = ripemd160();
        Assert.assertEqual(algorithm.blockSize, 512);
        Assert.assertEqual(algorithm.outSize, 160);
    ...
    it('should support sha512', function () {
        ...
        Assert.assertEqual(algorithm.blockSize, 1024);
        Assert.assertEqual(algorithm.outSize, 512);
    ...
    it('should support sha384', function () {
        ...
        Assert.assertEqual(algorithm.blockSize, 1024);
        Assert.assertEqual(algorithm.outSize, 384);
